...
def diff_encode(fname,golomb_m=512):
    ...
    # Write new file
    out = wave.open(fname+"_encoded_m{}_lossy.wav".format(golomb_m), "w")
...
def diff_decode(fname,golomb_m=512):
    ...
    while i < len(data):
        ...
        if readingLeft:
            d_left = int( val * math.pow(2,numberOfBitsOriginal - numberOfBitsNew) )
            valLeft = d_left+prevLeft
            prevLeft = valLeft
            readingLeft = False
        else:
            d_right = int( val * math.pow(2,numberOfBitsOriginal - numberOfBitsNew) )
